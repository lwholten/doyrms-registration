...
function signOutUser($userID, $locationID, $eventID, $staffAction, $staffMessage) {
  global $ini;

  // Associative array containing the required SQL queries
  $queries = array(
    // Inserts a 'sign out log' into the logs table
    "insert" => "INSERT INTO Log ( UserID, LocationID, LogTime, EventID, MinutesLate, Auto, StaffAction, StaffMessage ) VALUES (?, ?, CURRENT_TIMESTAMP, ?, 0, 0, ?, ?)",
    // Updates the users curent location
    "update" => "UPDATE Users SET LastActive=CURRENT_TIMESTAMP, LocationID=? WHERE UserID=?"
  );
  // Connects to the database
  $con = new mysqli($ini['db_hostname'], $ini['db_user'], $ini['db_password'], $ini['db_name']);
  // Prepares and executes the insert statement
  $stmt = $con->prepare($queries['insert']);
  $stmt->bind_param("iiiis", $userID, $locationID, $eventID, $staffAction, $staffMessage);
  $stmt->execute();
...
// Note that this will trigger if the user is signed in but has not signed off for the event
  ...
  // Sign the user in
  if (signOutUser($userID, $locationID, $eventID, $staffAction, $staffMessage)) {

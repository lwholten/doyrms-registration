...
// Main
function populateActivityTable($userID) {
  global $ini;
  // Connects to the database
  $con = new mysqli($ini['db_hostname'], $ini['db_user'], $ini['db_password'], $ini['db_name']);
  // Variable to store table contents
  $tableContents = '';
  // SQL code to get the table data
  /* Notes,
  when record $record[0] = n
  n = 0 -> user signed in
  n = 1 -> user signed out/var/www/html/doyrms-registration/pages/php/tables/staff/search_user_table.php
  ...
  // If userID has been set, filter by userID
  if (!is_null($userID)) {
    $sql .= "WHERE Log.UserID=$userID ";
  }
  // Add the ORDER clause to the end
  $sql .= "ORDER BY LogTime DESC LIMIT 25)";
  ...
  while($record = $result -> fetch_array(MYSQLI_NUM)) {
    ...
    // If the table is being filtered by userID, disable clicking on users
    if (isset($userID)) {
      $clickID = NULL;
    }
    // Otherwise, allow clicking on users to filter by said user
    else {
      $clickID=$record[9];
    }
    ...
    if ($record[0] == 0) {
      ...
      $tableContents .= formatRow($columns, $clickID);
    }
    // User signed out
    else if ($record[0] == 1) {
      $columns = [
...
// If ajax is being used and userID is not set, assume all users, return JSON encoded table contents
if (isset($_POST['ajax']) && $_POST['ajax'] == true && !isset($_POST['userid'])) {
  echo json_encode(populateActivityTable(NULL));
}
// If ajax is being used and userID is set, assume that user, return JSON encoded table contents
else if (isset($_POST['ajax']) && $_POST['ajax'] == true && isset($_POST['userid'])) {
  echo json_encode(populateActivityTable($_POST['userid']));
}
// If not, we assume file is being required/included and the function is being executed elsewhere
?>

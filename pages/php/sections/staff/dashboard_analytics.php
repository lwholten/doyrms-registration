<?php
function fillDashboardAnalytics() {
  function users() {
...
';
    ...
    while($record = $result -> fetch_array(MYSQLI_NUM)) {
      ...
      echo "<container class='dashboard_wrapper'><div class='user_analytics'><h5>Users</h5>";
      echo "<h6>Total Users: $record[0]</h6>";
      // Outputs the user bar
      echo "<spacer></spacer><div class='user_bar'>";
      foreach($barData as $data){
        if ($data[0] != 0) {
          echo "<span style='width: $data[0]%' class='$data[1]'></span>";
        }
      }
      // Outputs the legend for the user bar
      echo "</div><ul class='user_bar_legend'>";
      foreach($legendValues as $value){
        if ($value[2] != 0) {
          echo "<li><h6><span class='inline-bar $value[0]'></span>$value[2] $value[1]</h6></li>";
        }
      }
      // Ends the 'users' part
      echo "</ul></div></container><divider></divider>";
  ...
  function activity() {
    ...
    while($record = $result -> fetch_array(MYSQLI_NUM)) {
      ...
      echo "<container class='dashboard_wrapper'><div class='activity_analytics'><h5>Todays Activity</h5><spacer></spacer><ul>";
      // Outputs each metric
      foreach($metricValues as $value){
        echo "<li><h5>$value[0]</h5><h6>$value[1]</h6></li>";
      }
      // Ends the 'activity' part
      echo "</ul></div></container>";
